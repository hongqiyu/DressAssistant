...
public class ChangeTele extends AppCompatActivity {
    ...
    private void updatePhone(String strPhoneNum,String strUserName){
        if(isStrEmpty(strPhoneNum) == false){
            if(db!=null){
                ContentValues cvPhone = new ContentValues();
                cvPhone.put("pers_Phone",strPhoneNum);
                db.update("PersInfo",cvPhone,"pers_UsID = ?",new String[] { strUserName });
                Toast.makeText(ChangeTele.this,"修改成功！",Toast.LENGTH_SHORT).show();
                Intent intent = new Intent(ChangeTele.this,Myself.class);
                intent.putExtra("PhoneNum",strPhoneNum);
                startActivity(intent);
            }
        }
        else {
            Toast.makeText(ChangeTele.this,"新手机号不能为空！",Toast.LENGTH_SHORT).show();
        }
    }
